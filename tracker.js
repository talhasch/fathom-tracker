(function () {
  ...
  function getCookie(name) {
    var expires = localStorage.getItem(name + '__expires__');

    if (expires) {
      expires = parseInt(expires);
      if (expires && expires < new Date().getTime()) {
        localStorage.removeItem(name);
        localStorage.removeItem(name + '__expires__');
        return '';
      }
    }
  ...
  function setCookie(name, data, args) {
    ...
    if (args.expires) {
      localStorage.setItem(name + '__expires__', args.expires.getTime().toString());
    }
  }
